#!/bin/bash
# Train query-based frame selection plus the task LoRA adapters.
# Compared with train_query_selection.sh:
#   - Loads TASK_LORA_PATH as trainable LoRA instead of merging it frozen
#   - Trains query_selection MLPs fully
#   - Keeps vision tower, spatial tower, fusion block, and base LLM weights frozen
...
# Model: base VLM + trainable task LoRA (VLM-3R)
...
CHECKPOINT_OVERLAY_MEMORY_ONLY="${CHECKPOINT_OVERLAY_MEMORY_ONLY:-True}"
...
# Multinode: set NNODES and NODE_RANK (train_query_selection_lora.slurm does this via srun)
NNODES=${NNODES:-${SLURM_NNODES:-1}}
NODE_RANK=${NODE_RANK:-0}
NUM_GPUS_PER_NODE=${NUM_GPUS_PER_NODE:-${NUM_GPUS:-1}}

# Training. Defaults match query-only training: effective batch 64, one epoch,
# save every 50 steps with at most 20 checkpoints.
...
# Data: same ScanNet VSI training split used by query-only training.
...
RUN_LABEL="${RUN_LABEL:-128to32}"
RUN_NAME="${RUN_NAME:-vlm2-query-selection-lora-${RUN_LABEL}-${RUN_STAMP}}"
...
echo "=========================================="
echo "Query selection + task LoRA training"
echo "=========================================="
echo "Model: ${MODEL_PATH}"
echo "Task LoRA: ${TASK_LORA_PATH}"
echo "Checkpoint adapter: ${CHECKPOINT_ADAPTER:-<none>} (memory_only=${CHECKPOINT_OVERLAY_MEMORY_ONLY})"
echo "Query selection: num_select=${QUERY_SELECTION_NUM_SELECT}, temp=${QUERY_SELECTION_TEMPERATURE}, project_selected=${QUERY_SELECTION_PROJECT_SELECTED}, disable_dual_memory=${QUERY_SELECTION_DISABLE_DUAL_MEMORY}"
echo "Dual memory: use=${USE_DUAL_MEMORY}, mode=${MEMORY_MODE}, L_w=${MEMORY_L_W}, L_e=${MEMORY_L_E}, salience_gate=${EPISODIC_MEMORY_GATED_ATTENTION}"
...
ACCELERATE_CPU_AFFINITY=0 torchrun \
    ...
    $([[ -n "${CHECKPOINT_ADAPTER}" ]] && echo --checkpoint_adapter "${CHECKPOINT_ADAPTER}") \
