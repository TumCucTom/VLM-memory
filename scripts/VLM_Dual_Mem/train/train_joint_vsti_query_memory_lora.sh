#!/bin/bash
# Jointly train:
#   1. task LoRA adapters
#   2. query-selection MLPs
#   3. dual-memory weights
#
# Warm start:
#   - TASK_LORA_PATH points to a query-selection + VSTI LoRA checkpoint
#   - CHECKPOINT_ADAPTER overlays trained memory weights only
...
TASK_LORA_PATH="${TASK_LORA_PATH:-$HOME/scratch/VLM-memory/work_dirs/vlm2-query-selection-lora-vsti-memory400-raw-128to32-4460032/checkpoint-2050}"
CHECKPOINT_ADAPTER="${CHECKPOINT_ADAPTER:-$HOME/scratch/VLM-memory/work_dirs/vlm2-full-memory-vsti-no-salience-Lw8-Le32-32f-4461527/checkpoint-1000}"
...
USE_DUAL_MEMORY="${USE_DUAL_MEMORY:-True}"
...
QUERY_SELECTION_DISABLE_DUAL_MEMORY="${QUERY_SELECTION_DISABLE_DUAL_MEMORY:-False}"
...
RUN_NAME="${RUN_NAME:-vlm2-joint-vsti-query-memory-lora-128to32-${RUN_STAMP}}"
...
echo "=========================================="
echo "Joint VSTI training: LoRA + query selection + dual memory"
echo "=========================================="
echo "Model: ${MODEL_PATH}"
echo "Task LoRA warm start: ${TASK_LORA_PATH}"
echo "Memory checkpoint overlay: ${CHECKPOINT_ADAPTER}"
echo "Memory overlay only: ${CHECKPOINT_OVERLAY_MEMORY_ONLY}"
...
ACCELERATE_CPU_AFFINITY=0 torchrun \
    ...
    --mm_tunable_parts "dual_memory,query_selection" \
...
echo "=========================================="
echo "Joint training finished. Checkpoints: ${OUTPUT_DIR}"
echo "=========================================="
